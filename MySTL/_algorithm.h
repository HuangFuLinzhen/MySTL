...
namespace mystl {
    ...
    inline void __push_heap_aux(RandomAccessIterator first, RandomAccessIterator last,
        Distance*, T*) {
        mystl::__push_heap(first, Distance((last - first) - 1), Distance(0), T(*(last - 1)));
    }

    template <class RandomAccessIterator>
    inline void push_heap(RandomAccessIterator first, RandomAccessIterator last) {
        //新元素已经位于底部容器的最尾端
        mystl::__push_heap_aux(first, last, difference_type(first), value_type(first));
    }
   ...
   //将最大值放到末尾
    ...
    void __adjust_heap(RandomAccessIterator first, Distance holeIndex,
        ...
        mystl::__push_heap(first, holeIndex, topIndex, value);
    }
    ...
    inline void __pop_heap(RandomAccessIterator first, RandomAccessIterator last,
        ...
        *result = *first;
        mystl::__adjust_heap(first, Distance(0), Distance(last - first), value);//value 暂时保存
    }

    template<class RandomAccessIterator, class T>
    inline void __pop_heap_aux(RandomAccessIterator first, RandomAccessIterator last, T*) {
        mystl::__pop_heap(first, last - 1, last - 1, T(*(last - 1)), difference_type(first));
    }

    template<class RandomAccessIterator>
    inline void pop_heap(RandomAccessIterator first, RandomAccessIterator last) {
        mystl::__pop_heap_aux(first, last, value_type(first));
    }
    ...
    void __make_heap(RandomIterator first,
        ...
        while (true) {//调整整个树的所有父节点
            mystl::__adjust_heap(first, parent, len, T(*(first + parent)));
            if (parent == 0) return;
            parent--;
        }
    }
    ...
    inline void make_heap(RandomIterator first, RandomIterator last) {
        mystl::__make_heap(first, last, value_type(first), difference_type(first));
    }
